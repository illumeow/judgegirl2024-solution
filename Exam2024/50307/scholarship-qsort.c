...
int cmp(const void *A, const void *B){
    ...
    if(!fcmp(s1->gpa_avg, s2->gpa_avg) &&
       s1->gpa_avg > s2->gpa_avg) return -1;
    else if(fcmp(s1->gpa_avg, s2->gpa_avg) &&
            !fcmp(s1->comp_avg, s2->comp_avg) &&
            s1->comp_avg > s2->comp_avg ) return -1;
    else if(fcmp(s1->gpa_avg, s2->gpa_avg) &&
            fcmp(s1->comp_avg, s2->comp_avg) &&
            !fcmp(s1->elec_avg, s2->elec_avg) &&
            s1->elec_avg > s2->elec_avg) return -1;
    else if(fcmp(s1->gpa_avg, s2->gpa_avg) &&
            fcmp(s1->comp_avg, s2->comp_avg) &&
            fcmp(s1->elec_avg, s2->elec_avg) &&
            s1->major > s2->major) return -1;
    else if(fcmp(s1->gpa_avg, s2->gpa_avg) &&
            fcmp(s1->comp_avg, s2->comp_avg) &&
            fcmp(s1->elec_avg, s2->elec_avg) &&
            s1->major == s2->major &&
            s1->total > s2->total) return -1;
    return 1;                                                              
}
...
void scholarship(int n, struct Student students[]){
    ...
    printf("%d\n", num_of_scholor);
    for(int i = 0; i < num_of_scholor; i++){
        printf("%s\n", students[i].id);
    }
}